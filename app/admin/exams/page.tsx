...
export default async function AdminExamsPage() {
  ...
  const exams = await prisma.exam.findMany({
    ...
    select: {
      id: true, title: true, slug: true, status: true, isPremium: true,
      year: true, category: true, fileUrl: true, createdAt: true,
    ...
    <main className="min-h-screen bg-slate-50">
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 mt-2">
          ...
          <p className="mt-1 text-slate-500">Suivez les épreuves, leur publication et leurs documents.</p>
        </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {exams.map((e) => (
                  <tr key={e.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-5 py-4">{e.year ?? "—"}</td>
                    <td className="px-5 py-4">{e.category ?? "—"}</td>
                    <td className="px-5 py-4">{e.status}</td>
                    <td className="px-5 py-4">{e.fileUrl ? "Disponible" : "À ajouter"}</td>
                  </tr>
                ))}
                {exams.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-5 py-12 text-center text-slate-500">Aucune épreuve pour le moment.</td>
                  </tr>
                )}
              </tbody>
            </table>
